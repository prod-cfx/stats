...
const EXTRA_ENV_IGNORED_DIRS = new Set(['.git', 'node_modules', '.nx', 'dist', 'logs', 'tmp'])
const EXTRA_ENV_ALLOWED_PATHS = new Set(['docker/.env', 'docker/.env.example'])
const LOCAL_ALLOWLIST_CONFIG = join(ROOT_DIR, 'scripts/config/local-env-allowlist.jsonc')
const EXEMPTED_KEYS_CONFIG = join(ROOT_DIR, 'scripts/config/exempted-keys.jsonc')
