...
命令:
  ...
  deploy [环境标志]          应用数据库迁移（prisma migrate deploy）
    环境标志: --dev, --staging, --prod, --test, --e2e（默认 --dev）

  contracts [generate]       生成 API 合约（OpenAPI -> Zod）
    导出 backend OpenAPI 规范并生成 packages/api-contracts
...
示例:
  ...
  dx deploy --dev           # 应用开发环境数据库迁移
  dx contracts               # 生成 API 合约
...
提示: 可通过 ./scripts/dx build <target> 分别构建受影响应用。
`)
      return
...
功能:
  应用数据库迁移（prisma migrate deploy）
  与 dx db deploy 功能相同，是顶级快捷命令

常见示例:
  dx deploy --dev      # 应用开发环境已有迁移
  dx deploy --staging  # 应用预发环境迁移
  dx deploy --prod     # 应用生产环境迁移（需确认）
`)
      return
    ...
    case 'contracts':
      console.log(`
contracts 命令用法:
  dx contracts [generate]

功能:
  导出 backend OpenAPI 规范并生成 API 合约（Zod 客户端）
  生成位置: packages/api-contracts/src/generated/backend.ts

常见示例:
  dx contracts          # 生成 API 合约
  dx contracts generate # 显式调用生成
`)
      return
