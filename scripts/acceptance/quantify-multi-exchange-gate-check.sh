...
check_provider_once() {
  ...
  curl -fsS "http://localhost:${PORT}/api/v1/market/quote?symbol=${symbol}" > "$quote_file"
  curl -fsS "http://localhost:${PORT}/api/v1/market/bars?symbol=${symbol}&timeframe=1h&limit=10" > "$bars_file"
...
PY
  ...
  psql -At -F $'\t' -v symbol="$symbol" -v t0="$t0" -v provider="$provider" "$QUANTIFY_DATABASE_URL" -c "
