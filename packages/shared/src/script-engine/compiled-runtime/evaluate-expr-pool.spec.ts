...
describe('evaluateExprPool', () => {
  ...
  describe('IN_TIME_WINDOW series kind (Phase 1 strategy.time_window gate)', () => {
    it('returns 1 when ctx.timestamp falls inside the configured window in the given timezone', () => {
      const exprPool = [
        {
          id: 'expr_in_tw',
          ...
          payload: {
            ...
            windows: [{ start: '09:30', end: '15:00' }],
          },
        },
      ]
      // 2026-05-07 02:00 UTC == 10:00 Asia/Shanghai (within 09:30-15:00)
      const ts = Date.UTC(2026, 4, 7, 2, 0, 0)
      const values = evaluateExprPool({ timestamp: ts, bars: [] }, exprPool, ['expr_in_tw'])
      expect(values.expr_in_tw).toBe(1)
    })

    it('returns 0 when ctx.timestamp falls outside the configured window', () => {
      const exprPool = [
        {
          id: 'expr_in_tw',
          nodeType: 'series' as const,
      ...
      // 2026-05-07 00:30 UTC == 08:30 Asia/Shanghai (before window)
      const ts = Date.UTC(2026, 4, 7, 0, 30, 0)
      const values = evaluateExprPool({ timestamp: ts, bars: [] }, exprPool, ['expr_in_tw'])
      expect(values.expr_in_tw).toBe(0)
    })

    it('returns null on invalid IANA timezone (no throw to upstream)', () => {
      const exprPool = [
        {
          id: 'expr_in_tw',
          nodeType: 'series' as const,
      ...
      const ts = Date.UTC(2026, 4, 7, 2, 0, 0)
      const values = evaluateExprPool({ timestamp: ts, bars: [] }, exprPool, ['expr_in_tw'])
      expect(values.expr_in_tw).toBeNull()
    })

    it('returns null when timestamp is missing from context', () => {
      const exprPool = [
        {
          id: 'expr_in_tw',
          nodeType: 'series' as const,
      ...
      const values = evaluateExprPool({ bars: [] }, exprPool, ['expr_in_tw'])
      expect(values.expr_in_tw).toBeNull()
    })

    it('returns null when windows array is empty or missing', () => {
      const exprPool = [
        {
          id: 'expr_in_tw',
          nodeType: 'series' as const,
