...
当作为 multi-agent 系统的执行层工具时（如被 pr-fix Agent 调用），需注意：

### Structured Output 支持

在 prompt 中明确要求 JSON 格式输出，例如：
...
必须返回 JSON 格式：
{
  "fixedIssues": [{"findingId": "SEC-001", "description": "修复说明"}],
  "rejectedIssues": [{"findingId": "PERF-001", "reason": "拒绝理由"}],
  "commits": [{"sha": "abc123", "message": "提交信息"}]
}
EOF
```
...
每个修复必须关联原问题的 `findingId`，用于追溯修复效果：

- `findingId` 来自调用方传递的 `issuesToFix[].id`
- 格式如 `SEC-001`、`PERF-002`、`QUAL-003`

### Context Isolation

- Codex 在独立进程中执行，自动实现上下文隔离
- 每次调用使用新的 session（除非显式 resume）
