...
export function isE2eFlagEnabled(key: string): boolean {
  return getE2eEnv().bool(key, false)
}
...
export function assertSafeE2eDatabase(label: string): void {
  const env = getE2eEnv()
  const appEnv = env.raw('APP_ENV')
  const databaseUrl = env.str('DATABASE_URL', '') ?? ''

  if (appEnv !== 'e2e' || (!databaseUrl.includes('e2e') && !databaseUrl.includes('test'))) {
    throw new Error(`${label}: unsafe database environment for E2E test: APP_ENV=${appEnv}, DATABASE_URL=${databaseUrl}`)
  }
}
