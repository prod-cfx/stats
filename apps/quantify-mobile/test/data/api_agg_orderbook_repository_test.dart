...
void main() {
  group('ApiAggOrderbookRepository.buildMarketData', () {
    test('maps asks/bids levels with venue source', () {
      final AggMarketData d =
          ApiAggOrderbookRepository.buildMarketData(_dto());
    ...
    test('venues → exchanges + exchangeMap', () {
      final AggMarketData d =
          ApiAggOrderbookRepository.buildMarketData(_dto());
      expect(d.exchanges.map((AggExchange e) => e.key), <String>['binance', 'okx']);
      expect(d.exchangeMap['binance']!.letter, 'B');
    });

    test('OI/volume reuse mock fixture constants (tracked #2269)', () {
      final AggMarketData d =
          ApiAggOrderbookRepository.buildMarketData(_dto());
      expect(d.oiCoins, isNotEmpty);
      expect(d.volData, isNotEmpty);
      expect(d.precisions, isNotEmpty);
    });
