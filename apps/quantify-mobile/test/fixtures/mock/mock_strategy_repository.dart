...
class MockStrategyRepository implements StrategyRepository {
  ...
  static StrategyMarketStats _statsFor(StrategyCard c) {
    ...
    // 衍生品事件 / 风控稳健类策略回撤偏小，其他类回撤偏大。
    final bool lowVol =
        c.category == StrategyCategory.derivativeEvent ||
        c.category == StrategyCategory.riskRobust;
  ...
  Future<StrategyMarketPage> listMarket({
    ...
    final int end = (start + pageSize) > all.length
        ? all.length
        : start + pageSize;
    ...
    final List<StrategyMarketItem> items = pageItems
        .map(
          (StrategyCard c) => StrategyMarketItem(
            card: c,
  ...
  Future<StrategyDetail> getStrategyDetail(String id) async {
    ...
    final List<double> curve = List<double>.generate(
      60,
      (int _) => rng.nextDouble(),
    );
