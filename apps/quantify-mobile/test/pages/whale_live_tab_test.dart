...
void main() {
  testWidgets('Live tab 初始渲染 ≥1 条 QzWhaleRow (默认 BTC)', (
    WidgetTester tester,
  ) async {
  ...
  testWidgets('默认顶部交互区对齐设计稿：币种 tab + 搜索 + 关注推送 + 胜率 + 倒计时', (
    WidgetTester tester,
  ) async {
    final _FakeWhaleFeedRepository repo = _FakeWhaleFeedRepository();
    await _pump(tester, repo);
    ...
    expect(find.text('BTC'), findsAtLeastNWidgets(1), reason: '默认必须展示 BTC tab');
    expect(find.byTooltip('搜索币种'), findsOneWidget, reason: '设计稿右侧搜索入口应存在');

    expect(find.text('关注币种推送'), findsOneWidget);
    expect(find.text('胜率'), findsAtLeastNWidgets(1));
    expect(find.textContaining('秒后更新'), findsOneWidget);

    expect(find.byType(TextField), findsNothing, reason: '实时页设计稿无阈值输入');
    expect(find.text('≥ \$'), findsNothing);
    expect(find.text('≥ \$5M'), findsNothing);
    expect(find.text('创建监控'), findsNothing, reason: '创建监控属于监控子页，不属于顶层实时页');
    expect(
      find.text('LIVE'),
      findsNothing,
      reason: '设计稿 coin tabs 行无 LIVE badge',
    );
  });
  ...
  testWidgets('默认 BTC chip 下所有 row 都是 BTC', (WidgetTester tester) async {
    ...
    final Iterable<QzWhaleRow> rows = tester.widgetList<QzWhaleRow>(
      find.byType(QzWhaleRow),
    );
    expect(rows.isNotEmpty, isTrue);
    for (final QzWhaleRow row in rows) {
      expect(
        row.event.symbol.startsWith('BTC'),
        isTrue,
  ...
  testWidgets('推流 emit 新 BTC 事件 → 顶部插入 + AnimatedContainer 高亮态', (
    ...
    expect(freshRow, findsOneWidget, reason: '新推流事件必须出现在 ListView 中');
    final QzWhaleRow inserted = tester.widget<QzWhaleRow>(freshRow);
    expect(inserted.highlight, isTrue, reason: '新插入的 row 应当处于高亮态');
    final QzWhaleRow topRow = tester.widget<QzWhaleRow>(
      find.byType(QzWhaleRow).first,
    );
    expect(topRow.event.id, fresh.id, reason: '新事件必须排在顶部');

    await tester.pump(const Duration(milliseconds: 750));
    final QzWhaleRow firstAfterDelay = tester.widget<QzWhaleRow>(
      find.byType(QzWhaleRow).first,
    );
    expect(firstAfterDelay.highlight, isFalse, reason: '700ms 后高亮应被清除');
  });
  ...
  testWidgets('过滤不匹配的推流事件不会进入列表', (WidgetTester tester) async {
    ...
    await tester.pump();
    final int afterEth = tester
        .widgetList<QzWhaleRow>(find.byType(QzWhaleRow))
        .length;
    expect(afterEth, beforeBtc, reason: 'ETH 不应在 BTC chip 下被插入');
  });

  testWidgets('离开 /live 页面后 stream 订阅被取消（无内存泄漏）', (WidgetTester tester) async {
    final _FakeWhaleFeedRepository repo = _FakeWhaleFeedRepository();
    ...
    for (
      int i = 0;
      i < 20 && find.byType(WhaleLiveTab).evaluate().isNotEmpty;
      i++
    ) {
  ...
  testWidgets('WhaleLiveTab 9 主题循环 pump 不抛异常', (WidgetTester tester) async {
  ...
  testWidgets('mock fixture 场景行内不出现 731 天前 / 天前 时间穿帮 (issue #1602)', (
    ...
    final bool hasNearLabel =
        find.textContaining('刚刚').evaluate().isNotEmpty ||
        find.textContaining('分钟前').evaluate().isNotEmpty ||
        find.textContaining('小时前').evaluate().isNotEmpty;
    expect(hasNearLabel, isTrue, reason: '应当看到近期相对时间标签（刚刚 / 分钟前 / 小时前）');
  });

  testWidgets(
    '推流相同 id 不会触发 Duplicate Key / ListView child order 异常 (issue #1603)',
  ...
  testWidgets('历史接口返回重复 id 时 ListView 也只渲染一次 (issue #1603)', (
    ...
    expect(
      find.byKey(ValueKey<String>(base.id)),
      findsOneWidget,
  ...
  testWidgets('胜率排序按钮可点击，不再弹禁用提示 (issue #1983)', (WidgetTester tester) async {
    final _FakeWhaleFeedRepository repo = _FakeWhaleFeedRepository(
      history: winSortHistory(),
    );
    await _pump(tester, repo);
    addTearDown(() async => repo.dispose());
