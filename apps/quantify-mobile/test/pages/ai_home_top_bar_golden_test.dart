...
/// 属性断言互补保留，不替换。
final Uri _testFile = Uri.parse('test/pages/ai_home_top_bar_golden_test.dart');
...
void main() {
  ...
  testWidgets('QzAiTopBar 两种态在 9 主题下渲染无异常', (tester) async {
    await verifyAllThemes(tester, () => Scaffold(appBar: _buildTopBar()), (
      t,
    ) async {
      expect(find.text('AI'), findsOneWidget);
    }, surfaceSize: _surfaceSize);
  ...
  testWidgets('QzAiTopBar 滚动后不叠加 Material3 scrolled-under 染色', (tester) async {
    await pumpQz(
      tester,
      Scaffold(
        appBar: _buildTopBar(
