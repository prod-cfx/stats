    ...
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-full bg-[color:var(--cf-surface-2)] flex items-center justify-center border border-[color:var(--cf-border)] overflow-hidden">
      ...
      <div className="flex items-center gap-3">
        <button 
          ...
          className={`w-full md:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-[color:var(--cf-surface)] border border-[color:var(--cf-border)] rounded-xl text-[color:var(--cf-text)] text-xs md:text-label font-medium hover:border-transparent hover:bg-gradient-to-r hover:from-primary hover:to-secondary active:scale-95 disabled:opacity-50 disabled:active:scale-100 transition-all group ${isRefreshing ? 'border-transparent bg-gradient-to-r from-primary to-secondary' : ''}`}
        >
          <RefreshCw className={`w-4 h-4 md:w-4.5 md:h-4.5 text-[color:var(--cf-muted)] group-hover:text-white transition-all ${isRefreshing ? 'animate-spin text-white' : ''}`} />
          <span className={`text-sm md:text-body font-bold text-[color:var(--cf-text-strong)] transition-colors`}>{isRefreshing ? t('whaleTracking.profile.header.refreshing') : t('whaleTracking.profile.header.realtimeData')}</span>
        </button>
      </div>
    </div>
  );
