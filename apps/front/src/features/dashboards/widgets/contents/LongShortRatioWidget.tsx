...
function ProgressBar({ long, short }: { long: number; short: number }) {
  return (
    <div className="relative w-full h-3 bg-[color:var(--cf-bg)] rounded-md overflow-hidden flex border border-[color:var(--cf-border)]">
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="text-[color:var(--cf-text-strong)] font-bold text-sm truncate">Long/Short Ratio</div>
      ...
      <div className="flex-1 overflow-y-auto min-h-0 cf-scrollbar pr-1 flex flex-col gap-4">
        <div className="rounded-xl border border-[color:var(--cf-border)] bg-[color:var(--cf-surface)] p-3 space-y-2 flex-none">
          <div className="flex items-center justify-between text-xs text-[color:var(--cf-muted)]">
            <span>Long <span className="text-[color:var(--cf-text-strong)] font-semibold">{summary.longPercent.toFixed(1)}%</span></span>
            <span>Short <span className="text-[color:var(--cf-text-strong)] font-semibold">{summary.shortPercent.toFixed(1)}%</span></span>
          </div>
          <ProgressBar long={summary.longPercent} short={summary.shortPercent} />
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="rounded-lg border border-[color:var(--cf-border)] bg-[color:var(--cf-surface-2)] p-2">
              <div className="text-[color:var(--cf-muted)]">Long (USD)</div>
              <div className="text-[#4ade80] font-semibold">{fmtUsd(summary.longAmountUsd)}</div>
            </div>
            <div className="rounded-lg border border-[color:var(--cf-border)] bg-[color:var(--cf-surface-2)] p-2">
              <div className="text-[color:var(--cf-muted)]">Short (USD)</div>
              <div className="text-[#ef4444] font-semibold">{fmtUsd(summary.shortAmountUsd)}</div>
            </div>
          </div>
        </div>

        <div className="rounded-xl border border-[color:var(--cf-border)] bg-[color:var(--cf-surface)] overflow-hidden flex-none">
          <div className="grid grid-cols-12 gap-2 px-3 py-2 text-[11px] uppercase tracking-widest text-[color:var(--cf-muted)] border-b border-[color:var(--cf-border)] bg-[color:var(--cf-surface)]">
          ...
          <div className="divide-y divide-[color:var(--cf-border)]">
            {rows.map((r) => (
              <div key={r.exchange} className="grid grid-cols-12 gap-2 px-3 py-2 text-sm hover:bg-[color:var(--cf-surface-hover)] transition-colors items-center">
                <div className="col-span-3 text-[color:var(--cf-text-strong)] font-medium truncate">{r.exchange}</div>
                <div className="col-span-5 flex flex-col gap-1">
                  <ProgressBar long={r.longPercent} short={r.shortPercent} />
                  <div className="flex items-center justify-between text-[10px] text-[color:var(--cf-muted)]">
