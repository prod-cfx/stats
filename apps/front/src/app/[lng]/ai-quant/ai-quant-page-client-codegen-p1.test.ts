...
describe('AiQuantPageClient codegen P1 guards', () => {
  ...
  it('preserves same-snapshot backtest results during reconciliation when backtest config still matches', () => {
    const next = applyCodegenResponseToConversationState({
      conversation: {
        ...createConversation((key: string) => key),
        id: 'conv-with-backtest',
        serverConversationId: 'server-conv-1',
        messages: [{ id: 'msg-1', role: 'assistant', content: 'Strategy code generated.' }],
        paramSchema: DEFAULT_PARAM_SCHEMA,
        paramValues: {
    ...
    expect(next.backtestResult).not.toHaveProperty('recoveryStatus')
