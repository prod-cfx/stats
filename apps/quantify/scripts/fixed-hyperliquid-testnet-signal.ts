...
import { parseFixedHyperliquidTestnetCliOptions } from '@/modules/strategy-signals/services/fixed-hyperliquid-testnet-signal-cli'
import { FixedHyperliquidTestnetSignalService } from '@/modules/strategy-signals/services/fixed-hyperliquid-testnet-signal.service'
...
@Module({
  ...
  providers: [
    ...
    {
      provide: FixedHyperliquidTestnetSignalService,
      useFactory: (
        prisma: PrismaService,
        env: EnvService,
        signalExecutor: SignalExecutorService,
      ) => new FixedHyperliquidTestnetSignalService(prisma, env, signalExecutor),
      inject: [PrismaService, EnvService, SignalExecutorService],
    },
  ],
})
class FixedHyperliquidTestnetSignalScriptModule {}

async function main() {
  const plan = parseFixedHyperliquidTestnetCliOptions(process.argv.slice(2))
  const app = await NestFactory.createApplicationContext(FixedHyperliquidTestnetSignalScriptModule, {
    logger: ['error', 'warn', 'log'],
  })

  try {
    const service = app.get(FixedHyperliquidTestnetSignalService)
    const results = []
    ...
    for (const step of plan.steps) {
        ...
        ? await service.createAndExecuteSignal({
            ...
            executionConfig: {
              ...
              execution: {
                ...
                enabled: true,
                dryRun: false,
                defaultQuoteAmount: Number(step.positionSizeQuote ?? '0'),
                minBalanceThreshold: 0,
                maxRiskFraction: 1,
              },
            },
          })
        : await service.createSignal(step)
      ...
      results.push({
        ...
        reason: step.reason,
        positionSizeQuote: step.positionSizeQuote ?? null,
      })
    }
