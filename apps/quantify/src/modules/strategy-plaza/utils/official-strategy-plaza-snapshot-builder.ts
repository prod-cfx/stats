...
function buildOfficialStrategyScript(template: OfficialStrategyPlazaTemplate): string {
  ...
  return [
    ...
    '  return { action, size: { mode: "RATIO", value: action === "CLOSE_LONG" ? 1 : Number(PARAMS.positionPct || 0) / 100 }, confidence: 90, reason, meta: { templateId: TEMPLATE_ID, parameterSearchId: PARAMS.parameterSearchId } }',
    '}',
    ...
    '    if (signal.exit) return decision("CLOSE_LONG", `official.${TEMPLATE_ID}.exit`)',
    '    if (signal.enter) return decision("OPEN_LONG", `official.${TEMPLATE_ID}.entry`)',
