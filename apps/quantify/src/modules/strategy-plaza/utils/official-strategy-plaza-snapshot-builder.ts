...
function buildOfficialStrategyScript(template: OfficialStrategyPlazaTemplate): string {
  ...
  return [
    ...
    '',
    'function closes(bars) { return bars.map(bar => Number(bar.close)) }',
    ...
    '}',
    'function previousSma(values, period) { return sma(values.slice(0, -1), period) }',
    'function std(values) {',
    ...
    '    const dif = fast[index] - slow[index]',
    '    if (dea == null) {',
    '      seed.push(dif)',
    ...
    '  return output',
    '}',
    'function riskForEntry(referencePrice) {',
    ...
    '}',
    'function decision(action, reason, referencePrice) {',
    '  const output = { action, size: { mode: "RATIO", value: action === "CLOSE_LONG" ? 1 : Number(PARAMS.positionPct || 0) / 100 }, confidence: 90, reason, meta: { templateId: TEMPLATE_ID, parameterSearchId: PARAMS.parameterSearchId } }',
    '  if (action === "OPEN_LONG") output.risk = riskForEntry(referencePrice)',
    '  return output',
    '}',
    'function evaluate(ctx) {',
    '  const bars = Array.isArray(ctx && ctx.bars) ? ctx.bars : []',
    ...
    '      return { enter: lastFast != null && lastSlow != null && currentFast > currentSlow && lastFast <= lastSlow, exit: currentFast < currentSlow }',
    '    }',
