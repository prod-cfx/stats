...
export class ClientOrderIdFactoryService {
  create(input: CreateClientOrderIdInput): string {
    if (input.exchangeId === 'hyperliquid') {
      return this.createHyperliquidClientOrderId(input)
    }
  ...
  private createHyperliquidClientOrderId(input: CreateClientOrderIdInput): string {
    const generated = `0x${createHash('sha256')
