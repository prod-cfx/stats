...
export interface CreateFixedBinanceTestnetSignalInput {
...
const DEFAULT_PERP_BASE_ASSET = 'XRP'
const DEFAULT_QUOTE_ASSET = 'USDT'
const DEFAULT_FIXED_USER_EMAIL = 'binance-testnet-fixed@local.dev'
...
export class FixedBinanceTestnetSignalService {
  ...
  isEnabled(): boolean {
    const raw = this.env.getString('QUANTIFY_FIXED_BINANCE_TESTNET_ENABLED')
    if (!raw) return false
  ...
  async fetchTickerPrice(symbol: string, marketType: MarketType): Promise<string> {
    const client = new BinanceClient(marketType, {
      apiKey: 'public-testnet',
      secret: 'public-testnet',
      isTestnet: true,
    })
  ...
  async resolveContext(): Promise<FixedBinanceTestnetSignalContext> {
    ...
    const strategyName = `FIXED-BINANCE-TESTNET-${spotSymbolCode}`
  ...
  async createSignal(input: CreateFixedBinanceTestnetSignalInput) {
    const context = await this.resolveContext()
    const entryPrice = input.entryPrice ?? await this.fetchTickerPrice(
      input.marketType === 'spot' ? context.spotSymbol : context.perpSymbol,
      input.marketType,
    )
  ...
  async createAndExecuteSignal(
    input: CreateFixedBinanceTestnetSignalInput & {
  ...
  private getSpotBaseAsset() {
    return (this.env.getString('QUANTIFY_FIXED_BINANCE_TESTNET_BASE_ASSET', DEFAULT_BASE_ASSET) ?? DEFAULT_BASE_ASSET).toUpperCase()
  }

  private getPerpBaseAsset() {
    return (
      this.env.getString('QUANTIFY_FIXED_BINANCE_PERP_TESTNET_BASE_ASSET', DEFAULT_PERP_BASE_ASSET)
      ?? DEFAULT_PERP_BASE_ASSET
    ).toUpperCase()
  }

  private getQuoteAsset() {
    return (this.env.getString('QUANTIFY_FIXED_BINANCE_TESTNET_QUOTE_ASSET', DEFAULT_QUOTE_ASSET) ?? DEFAULT_QUOTE_ASSET).toUpperCase()
  }
}
