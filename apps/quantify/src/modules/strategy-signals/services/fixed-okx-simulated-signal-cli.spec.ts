import { parseFixedOkxSimulatedCliOptions } from './fixed-okx-simulated-signal-cli'

describe('parseFixedOkxSimulatedCliOptions', () => {
  it('maps open-spot preset to an executable single-step entry action', () => {
    const plan = parseFixedOkxSimulatedCliOptions(['--preset', 'open-spot'])
  ...
  it('maps open-close-roundtrip preset with perp market to two executable steps', () => {
    const plan = parseFixedOkxSimulatedCliOptions([
      '--preset',
