...
export class SignalGeneratorService {
  ...
  /**
   * Phase 5 S5（#984）：dynamic_grid program lifecycle 跨 K 线状态（in-memory）。
   * 按 strategyInstanceId 桶 → { [programId]: ProgramLifecycleState }。
   * runOrderPrograms 第 8 参（programLifecycleStateIn）从此 map 取，输出
   * programLifecycleStateNext 写回此 map。
   * cleanupProgramLifecycleState() 由 strategy-instances.service 在 deleteInstance
   * 时调用清理。跨进程持久化（Redis 等）留 follow-up issue。
   */
  ...
  /**
   * Phase 5 S5（#984）：清理某策略实例的 dynamic_grid lifecycle 状态。
   * 由 strategy-instances.service 在 deleteInstance 时通过 EventEmitter2 通知，
   * 避免循环依赖；也可被外部直接调用。订阅在 onModuleInit 中注册。
   */
      ...
      return {
        adapter: {
          ...
          onBar(ctx) {
            ...
            // 让 follow-up 消费。
            // Phase 5 S5：dynamic_grid lifecycle 真实接入第 8 参 + 写回 service map
