...
describe('signalGeneratorService coordinator behavior', () => {
  ...
  function createService(overrides: Partial<any> = {}) {
    ...
    const generatorRepository = {
      ...
      findActiveWebhookSignalSubscriptions: jest.fn(),
      findAcceptedWebhookRuntimeEvents: jest.fn(),
    ...
    const service = new SignalGeneratorService(
      ...
      overrides.okxMarketDataProvider as any,
    )
  ...
  it('injects live webhook event streams into published runtime context', async () => {
    const { service, generatorRepository } = createService({
      generatorRepository: {
        findActiveWebhookSignalSubscriptions: jest.fn().mockResolvedValue([{ signalId: 'whale_buy' }]),
        findAcceptedWebhookRuntimeEvents: jest.fn().mockResolvedValue([
          {
            id: 'evt-live-1',
            signalId: 'whale_buy',
            payload: { signalId: 'whale_buy' },
            receivedAt: new Date(1_775_000_890_000),
    ...
    await (service as any).generatePublishedSnapshotRuntimeSignalOutcome(
      { id: 'instance-1', params: {} },
      {
        id: 'template-1',
        promptTemplate: 'AI_CODEGEN_PUBLISHED_TEMPLATE',
        script: '/* @generated by compiler.v1 */\nconst EXPR_POOL = [{"id":"expr_webhook","nodeType":"predicate","payload":{"kind":"externalSignal","params":{"provider":"webhook","signalId":"whale_buy","sourceFeedId":"webhook.whale_buy","ttlMs":60000}}}] as const',
      },
      {
        id: 'symbol-perp-1',
        code: 'BTCUSDT:PERP',
    ...
    expect(generatorRepository.findActiveWebhookSignalSubscriptions).toHaveBeenCalledWith({
      strategyInstanceId: 'instance-1',
      signalIds: ['whale_buy'],
    })
    expect(generatorRepository.findAcceptedWebhookRuntimeEvents).toHaveBeenCalledWith(expect.objectContaining({
      strategyInstanceId: 'instance-1',
      signalIds: ['whale_buy'],
      until: new Date(primaryCloseTs),
    }))
    expect(onBar.mock.calls[0]?.[0].eventInbox).toEqual({
      'webhook.whale_buy': [
        { id: 'evt-live-1', ts: 1_775_000_890_000, payload: { signalId: 'whale_buy' } },
      ],
    })
  })

  it('injects live funding external feed streams into published runtime context', async () => {
    const okxMarketDataProvider = {
      fetchFundingRateEvents: jest.fn().mockResolvedValue([
        { id: 'funding-live-1', ts: 1_775_000_890_000, payload: { fundingRate: 0.0001 } },
      ]),
      fetchLiquidationEvents: jest.fn(),
    }
    const { service, generatorRepository } = createService({ okxMarketDataProvider })
    const primaryCloseTs = 1_775_000_900_000
    jest.spyOn(service as any, 'loadRecentBars').mockResolvedValue([{
      open: 99,
      high: 101,
    ...
    const onBar = jest.fn().mockReturnValue({ action: 'NOOP', reason: 'test.noop' })
    jest.spyOn(service as any, 'buildCompiledRuntimeAdapter').mockReturnValue({ adapter: { onBar }, parseError: null })
    jest
      .spyOn((service as any).decisionStage, 'buildPublishedRuntimeSignalOutcomeFromDecision')
      .mockReturnValue({ kind: 'noop', reasonCode: 'SNAPSHOT_RUNTIME_EXECUTION_NO_SIGNAL', reason: 'test' })
    ...
    await (service as any).generatePublishedSnapshotRuntimeSignalOutcome(
      { id: 'instance-1', params: {} },
      {
        id: 'template-1',
        promptTemplate: 'AI_CODEGEN_PUBLISHED_TEMPLATE',
        script: '/* @generated by compiler.v1 */\nconst EXPR_POOL = [{"id":"expr_funding","nodeType":"predicate","payload":{"kind":"fundingRateCondition","params":{"schemaRef":"funding","sourceFeedId":"funding.rate","operator":"GT","value":0}}}] as const',
      },
      {
        id: 'symbol-perp-1',
        code: 'BTCUSDT:PERP',
    ...
    expect(generatorRepository.findActiveWebhookSignalSubscriptions).not.toHaveBeenCalled()
    expect(okxMarketDataProvider.fetchFundingRateEvents).toHaveBeenCalledWith({
      symbol: 'BTCUSDT:PERP',
      startMs: primaryCloseTs - 60_000,
    ...
    expect(onBar.mock.calls[0]?.[0].dataSourceFeeds).toEqual({
      'funding.rate': { schema: 'funding', permissionGranted: true, hasData: true },
    })
  })

  it('injects live orderbook and open interest external feed streams into published runtime context', async () => {
    const okxMarketDataProvider = {
      fetchOrderbookImbalanceEvents: jest.fn().mockResolvedValue([
        { id: 'book-live-1', ts: 1_775_000_890_000, payload: { bidDepth: 3, askDepth: 2 } },
      ]),
      fetchOpenInterestEvents: jest.fn().mockResolvedValue([
        { id: 'oi-live-1', ts: 1_775_000_890_000, payload: { openInterest: 123 } },
      ]),
    }
    const { service, generatorRepository } = createService({ okxMarketDataProvider })
    const primaryCloseTs = 1_775_000_900_000
    ...
    await (service as any).generatePublishedSnapshotRuntimeSignalOutcome(
      { id: 'instance-1', params: {} },
      {
        id: 'template-1',
        promptTemplate: 'AI_CODEGEN_PUBLISHED_TEMPLATE',
        script: '/* @generated by compiler.v1 */\nconst EXPR_POOL = [{"id":"expr_book","nodeType":"predicate","payload":{"kind":"orderbookImbalance","params":{"sourceFeedId":"orderbook.imbalance"}}},{"id":"expr_oi","nodeType":"predicate","payload":{"kind":"openInterestCondition","params":{"sourceFeedId":"open_interest"}}}] as const',
      },
      {
        id: 'symbol-perp-1',
        code: 'BTCUSDT:PERP',
    ...
    expect(generatorRepository.findActiveWebhookSignalSubscriptions).not.toHaveBeenCalled()
    expect(okxMarketDataProvider.fetchOrderbookImbalanceEvents).toHaveBeenCalledWith({
      symbol: 'BTCUSDT:PERP',
      startMs: primaryCloseTs - 60_000,
      endMs: primaryCloseTs,
    })
    expect(okxMarketDataProvider.fetchOpenInterestEvents).toHaveBeenCalledWith({
      symbol: 'BTCUSDT:PERP',
      startMs: primaryCloseTs - 60_000,
      endMs: primaryCloseTs,
    })
    expect(onBar.mock.calls[0]?.[0].eventInbox).toEqual({
      'orderbook.imbalance': [{ id: 'book-live-1', ts: 1_775_000_890_000, payload: { bidDepth: 3, askDepth: 2 } }],
      open_interest: [{ id: 'oi-live-1', ts: 1_775_000_890_000, payload: { openInterest: 123 } }],
    })
    expect(onBar.mock.calls[0]?.[0].dataSourceFeeds).toEqual({
      'orderbook.imbalance': { schema: 'orderbook', permissionGranted: true, hasData: true },
      open_interest: { schema: 'open_interest', permissionGranted: true, hasData: true },
    })
  })

  it('fails closed when published runtime webhook signal subscription is inactive', async () => {
    const { service, generatorRepository } = createService({
      generatorRepository: {
    ...
    expect(generatorRepository.findAcceptedWebhookRuntimeEvents).not.toHaveBeenCalled()
