...
describe('ATOM_CONTRACT_REGISTRY full PR1b registration', () => {
  it('registers all supported atoms with PR1b contract fields', () => {
    ...
    for (const [key, contract] of entries) {
      ...
      // PR3a Phase 2 + PR3e + Issue #1313 PR1/PR2：irShape capabilityStatus 多态共存，按字面量分流：
      ...
      //     'pr3e-lifecycle'                 → Issue #1313 接口骨架字面量；rule-level / spec-level
      ...
      //   兑现 'pr3e-rule-block' / 'pr3e-orchestration-portfolio' / 'pr3e-lifecycle' atom 后
      //   字面量联合自然扩大，分流仍持续有效。
