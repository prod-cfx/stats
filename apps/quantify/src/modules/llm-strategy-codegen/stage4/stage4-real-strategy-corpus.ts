...
export const STAGE4_REAL_STRATEGY_CORPUS = [
  ...
  },
  {
    ...
    clarificationTurns: [{ assistantSlotPath: 'exit_rule', userAnswer: '亏损 3% 止损。' }],
    expectedFailure: null,
  },
  {
  ...
  },
  {
    ...
    clarificationTurns: [{ assistantSlotPath: 'exit_rule', userAnswer: '跌破震荡区间下沿时平多。' }],
    expectedFailure: null,
  },
  {
  ...
  {
    id: 'stage4-boundary-atr-fixed-notional-leverage',
    category: 'mean_reversion',
    initialUserMessage: 'OKX 合约 BTCUSDT 15m，价格触及指标下边界时开多，每次固定 100 USDT，使用 5 倍杠杆，并用 ATR14 的 2 倍止损。',
    expectedAtomKeys: ['price.detect.indicator_boundary', 'action.open_long', 'position.fixed_notional', 'position.leverage', 'risk.atr_stop'],
    expectedSemanticIntent: ['indicator boundary entry', 'fixed notional sizing', 'leverage constraint', 'ATR stop'],
    clarificationTurns: [],
    expectedFailure: null,
  },
  {
    id: 'stage4-reduce-limit-conditional-order',
    category: 'action_lifecycle',
    initialUserMessage: 'OKX 合约 BTCUSDT 15m，RSI14 高于 70 时用限价单减仓 50%，突破 70000 后下条件单开多。',
    expectedAtomKeys: ['oscillator.rsi_gte', 'action.reduce_position', 'action.limit_order', 'action.conditional_order'],
    expectedSemanticIntent: ['reduce position lifecycle', 'limit order execution', 'conditional order trigger'],
    clarificationTurns: [],
    expectedFailure: null,
  },
  {
    id: 'stage4-program-twap',
    category: 'execution_program',
    initialUserMessage: 'OKX 合约 BTCUSDT 15m，EMA20 上穿 EMA50 后用 TWAP 分 10 次均匀买入，总仓位 1000 USDT。',
    expectedAtomKeys: ['indicator.cross_over', 'program.twap'],
    expectedSemanticIntent: ['TWAP execution program', 'trend gated execution'],
    clarificationTurns: [],
    expectedFailure: null,
  },
  {
    id: 'stage4-program-dca',
    category: 'execution_program',
    initialUserMessage: 'OKX 合约 BTCUSDT 1h，启用 DCA 程序，每回撤 3% 买入一次，最多执行 5 次。',
    expectedAtomKeys: ['program.dca'],
    expectedSemanticIntent: ['DCA execution program'],
    clarificationTurns: [{ assistantSlotPath: 'entry_rule', userAnswer: '策略启动后启用 DCA 程序。' }],
    expectedFailure: null,
  },
  {
    id: 'stage4-program-martingale',
    category: 'execution_program',
    initialUserMessage: 'OKX 合约 BTCUSDT 15m，开启马丁程序，亏损后按 2 倍补单，最多补 3 次。',
    expectedAtomKeys: ['program.martingale'],
    expectedSemanticIntent: ['martingale execution program'],
    clarificationTurns: [{ assistantSlotPath: 'entry_rule', userAnswer: '策略启动后启用马丁程序。' }],
    expectedFailure: null,
  },
  {
    id: 'stage4-program-rebalance',
    category: 'execution_program',
    initialUserMessage: 'OKX 现货 BTC 和 ETH 做 50% 50% 等权组合，每天执行 rebalance 再平衡。',
    expectedAtomKeys: ['program.rebalance'],
    expectedSemanticIntent: ['rebalance execution program'],
    clarificationTurns: [{ assistantSlotPath: 'entry_rule', userAnswer: '每天固定时间执行再平衡程序。' }],
    expectedFailure: null,
  },
  {
    id: 'stage4-program-iceberg',
    category: 'execution_program',
    initialUserMessage: 'OKX 合约 BTCUSDT 15m，突破 70000 后用 iceberg 冰山单买入 1000 USDT，拆成每次 100 USDT。',
    expectedAtomKeys: ['program.iceberg'],
    expectedSemanticIntent: ['iceberg execution program'],
    clarificationTurns: [{ assistantSlotPath: 'entry_rule', userAnswer: '价格突破 70000 后执行冰山单。' }],
    expectedFailure: null,
  },
  {
    id: 'stage4-multi-leg-pair-spread',
    category: 'multi_symbol',
    initialUserMessage: 'OKX 合约 BTCUSDT 和 ETHUSDT 做多空双腿价差，BTC 腿做多、ETH 腿做空，价差扩大时开仓。',
    expectedAtomKeys: ['scope.leg'],
    expectedSemanticIntent: ['multi-leg pair spread scope'],
    clarificationTurns: [{ assistantSlotPath: 'exit_rule', userAnswer: '价差回归到 0.2% 时平掉两条腿。' }],
    expectedFailure: null,
  },
  {
    id: 'stage4-indicator-slope-volume-confirmation',
    category: 'simple_trend',
    initialUserMessage: 'OKX 合约 BTCUSDT 15m，EMA20 斜率连续 3 根向上，成交量确认放大后开多。',
    expectedAtomKeys: ['indicator.slope', 'volume.confirmation', 'action.open_long'],
    expectedSemanticIntent: ['indicator slope predicate', 'volume confirmation predicate'],
    clarificationTurns: [{ assistantSlotPath: 'exit_rule', userAnswer: '跌破 EMA20 时平多。' }],
    expectedFailure: null,
  },
  {
    id: 'stage4-time-cooldown-window',
    category: 'simple_trend',
    initialUserMessage: 'OKX 合约 BTCUSDT 15m，MA20 上穿 MA50 开多，但每次交易后冷却窗口 30 分钟内不再开仓。',
    expectedAtomKeys: ['indicator.cross_over', 'time.cooldown_window', 'action.open_long'],
    expectedSemanticIntent: ['time cooldown window predicate', 'trend entry'],
    clarificationTurns: [{ assistantSlotPath: 'exit_rule', userAnswer: '跌破 EMA20 时平多。' }],
    expectedFailure: null,
  },
