...
const NEW_IN_CONTEXT_EXAMPLES: readonly string[] = [
  ...
  '  正确（必须）：grid.range_rebalance 只能用合法 params：centerOffsetPct 数字 0.4、levels 数字 10、perGridSizing 数字 10、sideMode="both"、breakoutAction="stop"；不要输出中文枚举、百分号字符串或 centerOffsetPct=0。',
  '    rules: [{',
  '      "id": "entry-centered-grid", "phase": "entry", "sideScope": "both",',
  ...
  '      "effects": [{ "kind": "atom", "key": "grid.range_rebalance", "params": { "centerOffsetPct": 0.4, "levels": 10, "perGridSizing": 10, "sideMode": "both", "recycle": "true", "breakoutAction": "stop" } }],',
  ...
  '    rules: [',
  '      { "id": "entry-breakout", "phase": "entry", "sideScope": "long", "condition": { "kind": "atom", "key": "breakout.channel_high_break", "params": { "period": 1, "timeframe": "1h" } }, "effects": [{ "kind": "atom", "key": "action.open_long", "params": {} }], "evidence": { "text": "BTC 1h 突破前高开多" } },',
  '      { "id": "add-profit", "phase": "entry", "sideScope": "long", "condition": { "kind": "atom", "key": "price.percent_change", "params": { "basis": "entry_avg_price", "direction": "up", "valuePct": 3 } }, "effects": [{ "kind": "atom", "key": "action.add_position", "params": { "addMode": "profit_pct", "profitThreshold": 3, "sizing": { "kind": "ratio", "value": 0.5, "unit": "ratio" }, "maxLayers": 3 } }], "evidence": { "text": "盈利 3% 后加仓 50%，最多加 3 层" } }',
  '    ]',
