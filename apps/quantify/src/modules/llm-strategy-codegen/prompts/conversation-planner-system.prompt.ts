...
const NEW_IN_CONTEXT_EXAMPLES: readonly string[] = [
  ...
  '',
  '【S5 复合：长周期 MA gate + 回踩短周期 MA 后重新站上 sequence】',
  '  用户："ETH 日线在 MA120 上方时，只做多；价格回踩 MA20 后重新站上 MA20 买入"',
  '  rules: [{ "id": "entry-ma120-gate-ma20-retest-reclaim", "phase": "entry", "sideScope": "long",',
  '    "condition": { "kind": "and", "children": [',
  ...
  '        { "kind": "atom", "key": "indicator.above", "params": { "indicator": "ma", "period": 20, "timeframe": "1d" } } ]} ]},',
  '    "effects": { "actions": [{ "kind": "atom", "key": "action.open_long", "params": {} }], "risks": [], "positions": [], "orchestration": [], "programs": [] },',
  '    "evidence": { "text": "回踩 MA20 后重新站上 MA20 买入" } }]',
  '  错误：退化为单 atom indicator.above(ma, 20)；或拆 gate 与 sequence 为 2 条独立 rule。',
  '',
...
const COMPOSITIONAL_PATTERN_HINTS: readonly string[] = [
  ...
  '',
  '【回踩 MA(N) / 重新站上 MA(N) / 跌破 MA(N) 后重新站上 MA(N) / 重新站回 MA(N)】',
  '  → sequence([indicator.below(maN), indicator.above(maN)])（sequence 内部 below/above 同 period/timeframe）；',
  '    若同时有长周期 gate，用 and([gate, sequence([...])]) 合并到同一条 entry rule。',
  '    禁止退化为单 atom indicator.above(maN)（丢「先跌破」时序）；禁止拆 gate/sequence 为两条 rule。',
  '',
  '【方向准入语句 + 触发条件】',
