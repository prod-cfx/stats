...
export function buildStrategyCodegenSystemPrompt(helperSignatures: string): string {
  return [
    ...
    '需求和约束中的每一条 entryRules / exitRules，以及会影响运行时决策的 riskRules，都必须在代码中有明确对应实现。',
    '每一条 entryRules / exitRules / riskRules，都必须对应代码中的一个独立条件判断或执行分支，禁止多个规则共享同一个条件表达式。',
    '禁止将多个 exit 或 risk 规则合并为单一 if 条件；每条规则必须独立判断并独立触发行为。',
    '对于“连续 N 根 K 线”类规则，必须显式实现逐 bar 计数或序列判断逻辑，禁止使用单一条件近似替代。',
    '若某条规则实现复杂，必须优先用最直接方式实现（例如显式计数、逐条件判断），禁止因复杂度而省略或弱化。',
    '禁止遗漏任何已经明确的策略规则；如果规则已出现在需求或 checklist 中，就不能静默忽略。',
    ...
    '生成前先逐条检查需求和约束里的规则是否都已覆盖，确认每条规则都能在代码中找到对应逻辑。',
