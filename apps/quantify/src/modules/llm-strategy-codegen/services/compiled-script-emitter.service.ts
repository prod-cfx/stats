...
export class CompiledScriptEmitterService {
  emit(input: EmitCompiledScriptInput): string {
    ...
    // Phase 5 S2 (#1104): ORCHESTRATION_SCOPES 仅在非空时输出（兼容路径）
    //   - 单/0 scope 旧 ast 经新 emitter 输出与 main 字节等价
    //   - parser 用 readOptionalConst peek prefix 不消费空行，自然兼容
    const orchestrationScopesLine = (projection.orchestrationScopes ?? []).length > 0
      ? this.emitConst('ORCHESTRATION_SCOPES', projection.orchestrationScopes)
      : null

    // Phase 5 S11 (#1112): ORCHESTRATION_LEG_SCOPES 同形兼容路径
    //   - 单/0 leg 旧 ast 经新 emitter 输出与 main 字节等价
    //   - 顺序紧邻 ORCHESTRATION_SCOPES 之后、TOPOLOGY 之前
    const orchestrationLegScopesLine = (projection.orchestrationLegScopes ?? []).length > 0
      ? this.emitConst('ORCHESTRATION_LEG_SCOPES', projection.orchestrationLegScopes)
      : null
    ...
    return [
      ...
      this.emitConst('ORCHESTRATION_PORTFOLIO_RISKS', projection.orchestrationPortfolioRisks ?? []),
      ...(orchestrationScopesLine ? [orchestrationScopesLine] : []),
  ...
  buildProjection(input: EmitCompiledScriptInput): CompiledScriptProjection {
    ...
    return {
      ...
      decisionPrograms,
      orderPrograms,
      ...(input.ast.orchestrationPortfolioRisks ? { orchestrationPortfolioRisks: input.ast.orchestrationPortfolioRisks } : {}),
      ...
      ...(input.ast.orchestrationScopes && input.ast.orchestrationScopes.length > 0
        ? { orchestrationScopes: input.ast.orchestrationScopes }
        : {}),
      // Phase 5 S11 (#1112): scope.leg substrate
      ...(input.ast.orchestrationLegScopes && input.ast.orchestrationLegScopes.length > 0
        ? { orchestrationLegScopes: input.ast.orchestrationLegScopes }
        : {}),
