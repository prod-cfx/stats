...
export class SemanticContractReadinessService {
  ...
  evaluateRulesReadiness(rules: readonly SemanticRule[] | undefined): RulesReadinessSummary {
    ...
    for (const rule of rules) {
      ...
      // DCA schedule 自洽：interval + amount 自带 add_position，即 entry 信号 + position 载体，
      // 无需 action.open_long / action.open_short 即满足 entry + position 就绪。
      // eslint-disable-next-line atom-keys/no-atom-key-literal -- rules-only readiness 必须直接匹配 position.dca_schedule 自洽闭环语义。
      if (effectLeaves.some(leaf => leaf.key === 'position.dca_schedule')) {
        sawDcaSchedule = true
      }
    ...
    // DCA schedule 自洽闭环：entry 信号 + position 载体一体
    if (sawDcaSchedule) {
      summary.hasEntry = true
      summary.hasPosition = true
    }
  ...
  evaluateMainflowRulesReadiness(rules: readonly SemanticRule[] | null | undefined): MainflowRulesReadinessResult {
    ...
    const gridRuleIndexes = new Set(
      read.leaves
        .filter(leaf =>
          // eslint-disable-next-line atom-keys/no-atom-key-literal -- rules-only mainflow: grid.range_rebalance anywhere in the typed rule is the executable grid program contract.
          leaf.key === 'grid.range_rebalance'
          || (leaf.role === 'program' && leaf.key.startsWith('program.') && leaf.key.includes('grid')),
        )
        .map(leaf => leaf.ruleIndex),
    )
