...
export class SemanticSeedExtractorService {
  ...
  private buildRulesFromSeedNodes(
    ...
    const positionEffects = position?.sizing
      ? [{
          kind: 'atom' as const,
          key: 'position.sizing',
  ...
  private extractContextSlots(text: string): SeedContextSlots {
    ...
    const timeframe = timeframes[0] ?? this.extractFirstExecutionContextTimeframe(text)
    if (timeframe) {
  ...
  private pushMovingAverageCrossTrigger(segment: string, triggers: SeedTrigger[], seen: Set<string>): void {
    ...
    let previousCross: { indicator: 'ma' | 'ema'; fastPeriod?: number; slowPeriod?: number } | null = null
    for (const { clause, parentClause } of this.splitLogicClausesWithParent(segment)) {
      const cross = this.parseMovingAverageCrossClause(clause)
        ?? this.parseInheritedMovingAverageCrossClause(clause, previousCross)
        ?? this.parseGenericMovingAverageCrossClause(clause, segment)
      if (!cross) continue
      ...
      if (cross.indicator === 'ma' || cross.indicator === 'ema') {
        previousCross = {
          indicator: cross.indicator,
          ...(cross.fastPeriod !== undefined ? { fastPeriod: cross.fastPeriod } : {}),
          ...(cross.slowPeriod !== undefined ? { slowPeriod: cross.slowPeriod } : {}),
  ...
  private parseInheritedMovingAverageCrossClause(
    clause: string,
    previousCross: { indicator: 'ma' | 'ema'; fastPeriod?: number; slowPeriod?: number } | null,
  ): { indicator: 'ma' | 'ema'; direction: 'up' | 'down'; fastPeriod?: number; slowPeriod?: number } | null {
    if (!previousCross) return null
    const normalized = clause.replace(/\s+/gu, '')
    if (/(?:EMA|MA)\d{1,4}/iu.test(normalized)) return null
    const hasUpWord = /上穿|crossover|金叉/iu.test(normalized)
    const hasDownWord = /下穿|crossunder|死叉/iu.test(normalized)
    if (!hasUpWord && !hasDownWord) return null

    return {
      indicator: previousCross.indicator,
      direction: hasUpWord ? 'up' : 'down',
      ...(previousCross.fastPeriod !== undefined ? { fastPeriod: previousCross.fastPeriod } : {}),
      ...(previousCross.slowPeriod !== undefined ? { slowPeriod: previousCross.slowPeriod } : {}),
    }
  }
