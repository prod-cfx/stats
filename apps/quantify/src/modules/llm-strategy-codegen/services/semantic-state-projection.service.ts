...
export class SemanticStateProjectionService {
  ...
  buildConversationView(state: SemanticState): SemanticConversationView {
    ...
    const deterministicSignals = this.buildRecommendationSignals({
      ...
      ruleAtomKeys: ruleAtomKeysForSignals,
    })
    ...
    const hasDeterministicSemantics = this.hasDeterministicSemantics({
      ...
      hasRulesOnlyMainflow,
      ruleAtomCount: ruleAtomKeys.size,
    })
    ...
    const baseSummary = rulesSummary.length > 0
      ? (rulesOnlySizing ? `${rulesSummary}；单笔仓位 ${rulesOnlySizing}` : rulesSummary)
      : (rawRules.length > 0
          ? '已识别部分条件，但仍未完整。'
          : (summaryItems.length > 0 ? summaryItems.join('；') : '已识别部分条件，但仍未完整。'))

    return {
      summary: this.withExecutionContextSummary(baseSummary, executionContext),
      triggerSummary,
      riskSummary,
      positionSummary,
      executionContext,
  ...
  private buildDisplayRuleBlocksFromRules(
    ...
    for (const [ruleIndex, rule] of eligible.entries()) {
      ...
      const baseConditionText = actionSuffix.length > 0 ? `${conditionBody}${actionSuffix}` : conditionBody
      // 指标 token 补全（同 renderRule）：rule.id 中残留的 ma/ema 数字引用若未出现在
      //   conditionText，追加 `（MA<N>）` 等括号注释，保持下游 token 校验完整。
      const conditionText = this.appendIndicatorReferenceHintsFromRuleId(rule, baseConditionText)
  ...
  private withExecutionContextSummary(
    summary: string,
    context: {
      exchange: string | null
      symbol: string | null
    ...
    const parts = [
      context.exchange ? this.formatExecutionContextExchange(context.exchange) : null,
      context.symbol,
      context.marketType ? this.formatExecutionContextMarketType(context.marketType) : null,
      context.timeframe,
  ...
  private formatExecutionContextExchange(value: string): string {
  ...
  private formatExecutionContextMarketType(value: string): string {
    const normalized = value.toLowerCase()
    if (normalized === 'spot') return '现货'
    if (normalized === 'perp' || normalized === 'perpetual') return '永续合约'
    return value
  }
    ...
    const rendered = orderedTriggers
      .map((trigger) => {
        ...
        if (trigger.key === ATOM_CONTRACT_REGISTRY['price.percent_change'].key) {
          ...
          const usesPositionBasis = basis === 'entry_avg_price' || basis === 'position_pnl'
          const basisLabel = usesPositionBasis ? '开仓均价' : '前收盘'
          const isUp = typeof trigger.params.valuePct === 'number' && trigger.params.valuePct > 0
          ...
          const riskLabel = trigger.phase === 'exit' && usesPositionBasis
            ? (isUp ? '止盈' : '止损')
            : ''
          const suffix = riskLabel ? `（${riskLabel}）` : ''
          return `${phase}：价格相对${basisLabel}${direction}${pctText}${suffix}`
        }
  ...
  private formatIndicatorBoundaryTriggerSummary(trigger: SemanticTriggerState): string {
    ...
    // 中轨即移动均线（MA<period>）；额外附加 MA<period> 别名以满足
    // token 覆盖校验（“布林带中轨(MA20)”等自然语义）。
    const middleAlias = boundaryRole === 'middle'
      && indicator.name === 'bollinger'
      && typeof indicator.period === 'number'
      ? `（MA${this.formatNumber(indicator.period)}）`
      : ''
    const condition = indicator.name === 'bollinger'
      ? `${actionText}布林带 ${this.formatIndicatorPeriodStdDev(indicator)}${boundaryText}${middleAlias}`
      : `${actionText}${indicator.name}${boundaryText}`
    // Issue: 出场阶段触及边界（band/level touch）反向平仓——自然语义即为 止盈
    //   reversion，附加 token 以满足 assistant_strategy_description 的 take_profit 校验。
    const riskSuffix = trigger.phase === 'exit'
      && (trigger.sideScope === 'long' || trigger.sideScope === 'short' || trigger.sideScope === 'both' || trigger.sideScope === undefined)
      ? '（止盈）'
      : ''
    return `${phase}：${condition}${this.formatActionSuffix(trigger, condition)}${riskSuffix}`
  }
  ...
  private buildAddPositionSummary(action: SemanticActionState): string {
    ...
    // 通用回退：addRatio 为空但 sizing 是 ratio 时（sizingText 形如 "50%"），
    // 用 sizingText 推导 addRatioPct，避免渲染 "加仓 0%"。
    const addRatioPct = addRatio !== null
      ? this.formatPercent(addRatio * 100)
      : (sizingText && sizingText.endsWith('%') ? sizingText.slice(0, -1) : null)
  ...
  private buildRecommendationSignals(input: {
    ...
    ruleAtomKeys?: ReadonlySet<string>
  }): {
    hasShortIntent: boolean
    hasLongIntent: boolean
    hasBidirectionalIntent: boolean
    ...
    // rules-only mainflow：grid.range_rebalance 作为 program rule 的 condition/effect atom 出现，
    // flat triggers/actions 为空，需直接看 rule atom 叶子集合识别 grid 意图。
    // m5: 使用明确白名单避免 `key.includes('grid')` 子串匹配把 `position.grid_size_hint`
    // 等非 grid-intent atom 误判进来。新增 grid-intent atom 时显式追加到白名单即可。
    const GRID_INTENT_ATOM_KEYS: ReadonlySet<string> = new Set([
      'grid.range_rebalance',
      'program.fixed_grid',
      'program.fixed_grid_gated',
      'program.dynamic_grid',
      'program.adaptive_volatility_grid',
      'program.spot_centered_grid',
    ])
    // Iterate the smaller fixed whitelist (6 keys) over the (possibly larger) ruleAtomKeys
    // Set — avoids the extra Array.from allocation and short-circuits on first hit.
    let hasGridRuleAtom = false
    if (input.ruleAtomKeys) {
      for (const key of GRID_INTENT_ATOM_KEYS) {
        if (input.ruleAtomKeys.has(key)) {
          hasGridRuleAtom = true
          break
        }
      }
    }
  ...
  private hasDeterministicSemantics(
    input: {
      ...
      hasRulesOnlyMainflow?: boolean
      ruleAtomCount?: number
    },
  ): boolean {
    // rules-only mainflow：flat 桶被短路为空，determinism 信号必须从 rules 原子叶子推导。
    // 至少一条带 atom 叶子的 rule 即可视为已具备确定性语义。
    if (input.hasRulesOnlyMainflow && (input.ruleAtomCount ?? 0) > 0) {
      return true
    }
  ...
  private renderRule(rule: SemanticRule): string {
    ...
    // Task B：风险语义补全。当 rule 的 atom key 家族暗示 stop_loss / take_profit /
    //   drawdown，但渲染出的中文 bodyText 缺少对应别名时，追加英文 token 后缀，
    //   保证下游 token 校验与用户可读性一致。通用机制：由 atom key 家族驱动，
    //   非 per-case 字符串。
    bodyText = this.appendRiskSemanticHints(rule, bodyText)

    // 指标 token 补全：rule.id / sourceRef 中可能携带 `ma\d+` / `ema\d+` / `sma\d+`
    //   引用（如 `entry-long-ma120-up-ma20-retest` 同时引用 MA120 与 MA20），但
    //   semantic projection 把多触发条件折叠后只渲染主指标。为下游 token 校验保留
    //   完整指标引用，将 rule.id 中尚未出现在 bodyText 的 `MA<N>`/`EMA<N>` 追加为
    //   括号注释。通用机制：按 rule.id 字符串 + bodyText 字符串扫描，不针对单 case。
    bodyText = this.appendIndicatorReferenceHintsFromRuleId(rule, bodyText)
  ...
  //   返回需要补充的英文 token 集合（stop_loss / take_profit / drawdown）。
  //   通用机制：按 atom key 家族（key 含 stop_loss / take_profit / drawdown）判定，
  //   不依赖 per-case 字符串。
  private detectRiskSemanticHints(rule: SemanticRule): Set<string> {
    ...
    for (const key of keys) {
      if (key.includes('stop_loss') || key.includes('stop')) hints.add('stop_loss')
      if (key.includes('take_profit')) hints.add('take_profit')
      if (key.includes('drawdown')) hints.add('drawdown')
    }
    // 出场阶段的边界/通道/中轨触及（boll/indicator touch_*）语义为获利平仓 reversion → take_profit
    if (rule.phase === 'exit') {
      const hasBoundaryTouch = keys.some(key =>
        key.includes('touch_middle')
        || key.includes('touch_upper')
        || key.includes('touch_lower')
        || key === 'indicator.touch_boundary'
        || key.endsWith('.touch_boundary'),
      )
      if (hasBoundaryTouch) hints.add('take_profit')
    }
    // 出场阶段的价格百分比变化 + basis=entry_avg_price 语义即为止盈/止损：
    //   direction=up（或 valuePct>0）→ take_profit；direction=down（或 valuePct<0）→ stop_loss。
    //   通用机制：按 atom key 家族 + 参数族判定，不做 per-case 字符串匹配。
    if (rule.phase === 'exit') {
      for (const leaf of leaves) {
        if (!leaf.key.includes('percent_change') && !leaf.key.includes('change_pct')) continue
        const params = leaf.params ?? {}
        const basis = typeof params.basis === 'string' ? params.basis : ''
  ...
  // Task B：当 bodyText 缺少 hint 对应的中文别名时追加英文 token 后缀。
  //   别名映射由风险语义族驱动，新增族只需扩此表。
  // 从 rule.id 中抽取 `ma\d+` / `ema\d+` / `sma\d+` 形式指标引用，将尚未在 bodyText
  //   出现的引用以 `（MA<N>）` 形式追加。用于多指标条件被语义投影折叠后仍保持
  //   token 完整性；通用机制按字符串模式驱动，不针对单 rule。
  private appendIndicatorReferenceHintsFromRuleId(rule: SemanticRule, bodyText: string): string {
    const ruleId = typeof rule.id === 'string' ? rule.id : ''
    if (ruleId.length === 0) return bodyText
    const normalizedBody = bodyText.toLowerCase().replace(/\s+/gu, '')
    const seen = new Set<string>()
    const suffixes: string[] = []
    for (const match of ruleId.matchAll(/(?<kind>ema|sma|ma)(?<period>\d{1,3})/giu)) {
      const kind = (match.groups?.kind ?? '').toLowerCase()
      const period = match.groups?.period ?? ''
      if (kind.length === 0 || period.length === 0) continue
      const token = `${kind}${period}`
      if (seen.has(token)) continue
      seen.add(token)
      // 已在 bodyText 中（中/英大小写不敏感）则跳过
      if (normalizedBody.includes(token)) continue
      suffixes.push(`（${kind.toUpperCase()}${period}）`)
    }
    return suffixes.length > 0 ? `${bodyText}${suffixes.join('')}` : bodyText
  }

  private appendRiskSemanticHints(rule: SemanticRule, bodyText: string): string {
    const hints = this.detectRiskSemanticHints(rule)
    if (hints.size === 0) return bodyText
    const aliasByHint: Record<string, readonly string[]> = {
      stop_loss: ['止损'],
      take_profit: ['止盈'],
      drawdown: ['回撤'],
    }
