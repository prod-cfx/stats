...
describe('compiledPublicationGateService', () => {
  ...
  describe('grid orchestration does not override account positionMode', () => {
    const gate = new CompiledPublicationGateService(
    ...
    it('returns null for orchestration-only grid programs because sideMode is an order-program behavior', () => {
      const snapshot = {
        orchestration: {
          programs: [
      ...
      expect(privateGate.readCanonicalPositionMode(snapshot)).toBeNull()
    })

    it('keeps single-direction rules as long_only even when grid orchestration is present', () => {
      const snapshot = {
      ...
      expect(privateGate.readCanonicalPositionMode(snapshot)).toBe('long_only')
    })

    it('keeps perp_long order programs as long_only when combined with grid orchestration', () => {
      const snapshot = {
        orchestration: { programs: [{ programKind: 'dynamic_grid' }] },
    ...
    it('returns long_short only for real short exposure or neutral perp order programs', () => {
      const snapshot = {
        orchestration: { programs: [{ programKind: 'fixed_grid_gated' }] },
        orderPrograms: [{ kind: 'place_limit_grid', mode: 'perp_neutral' }],
      }
