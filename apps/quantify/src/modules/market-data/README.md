...
1. 在仓库根目录执行 `dx db generate`
2. 启动服务：`dx start backend --dev`
3. 在内部调用方连接 `http://localhost:3000/api/v1/market/stream/ticker`
...
- 连接失败：确认后端服务已经启动，且调用方可访问 `/api/v1/market/stream/ticker`
