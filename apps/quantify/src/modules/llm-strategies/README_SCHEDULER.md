...
- 服务重启时，当前只会恢复处于 `running` 状态、关联策略仍为 `live` 且数据库里显式保存了 `scheduleCron` 的实例
...
但当前实现不会在服务重启后恢复这类“依赖默认 Cron、数据库中 `scheduleCron` 为空”的实例。
...
- 支持热更新 Cron 表达式
- 支持服务重启后的任务恢复
  仅限显式配置了 `scheduleCron` 的 running 实例
- 支持手动触发，且手动触发可绕过调度限制
- 支持运行中并发保护
