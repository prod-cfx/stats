...
基于 Nx 的 Monorepo 项目，提供 **NestJS 后端**、**Next.js 用户端**、**Next.js 管理台** 以及共享包。所有应用使用 Nx 进行任务编排，开发/构建命令统一通过全局 `dx` 工具执行。
...
dx/                # dx 配置（commands/env-layers/env-policy）
ruler/             # AI 协作规范文档
docs/              # 项目文档
docker/            # Docker 部署配置
```
...
```bash
dx start all   # 并行启动 backend/front/admin（development）
```
...
> 部署 / CI 环境请统一使用 `dx db deploy --<env>`（如 `--staging`、`--prod`、`--test`），该命令会执行 `prisma migrate deploy`、无需 `--name`。
> `db seed` 会写入默认用户/管理员角色以及后台初始账号。如果跳过该步骤，前台用户注册时将无法为新账号分配 `USER` 角色，接口会返回 “Default user role is missing”。

## 环境变量策略

所有入口通过 `packages/config` 的 `loadEnvironment()` 统一加载，并由 `dx` 在命令入口做强校验：
...
所有命令从仓库根目录执行，统一通过 `dx` 调用：

| 类别       | 命令                                     | 说明                              |
| ---------- | ---------------------------------------- | --------------------------------- |
| **启动**   | `dx start backend --dev`                 | 启动 Nest 开发服务（3000）        |
|            | `dx start front --dev`                   | 启动用户端 Next（3001）           |
|            | `dx start admin --dev`                   | 启动管理端 Next（3500）           |
|            | `dx start all`                           | 并行启动所有服务                  |
| **构建**   | `dx build backend --dev`                 | Nest 构建                         |
|            | `dx build front --dev`                   | Next 用户端构建                   |
|            | `dx build admin --dev`                   | Next 管理端构建                   |
| **Lint**   | `dx lint`                                | 全量 lint 检查                    |
|            | `dx lint --fix`                          | 自动修复 lint 问题                |
| **数据库** | `dx db generate`                         | Prisma generate                   |
|            | `dx db format`                           | schema.prisma 格式化              |
|            | `dx db migrate --dev --name <name>`      | 开发迁移（必须提供 name）         |
|            | `dx db seed --dev`                       | 执行种子脚本                      |
|            | `dx db deploy --prod`                    | 应用迁移（prisma migrate deploy） |
| **合约**   | `dx build contracts --dev`               | 导出 Swagger + 生成 Zod Client    |
| **测试**   | `dx test e2e backend`                    | 运行全部 E2E 测试                 |
|            | `dx test e2e backend <file> [-t "case"]` | 指定文件/用例                     |
| **缓存**   | `dx cache clear`                         | 清理 Nx/PNPM 缓存                 |
