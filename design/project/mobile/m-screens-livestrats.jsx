...
/* sort metrics for the 筛选 & 排序 sheet (funnel button) */
const LS_SORTS = [
  { k:'todayPnl', label:'今日盈亏', get:s => s.todayPnl },
  { k:'totalPnl', label:'累计盈亏', get:s => s.totalPnl },
  { k:'totalPct', label:'收益率',   get:s => s.totalPct },
  { k:'winRate',  label:'胜率',     get:s => s.winRate },
  { k:'capital',  label:'投入本金', get:s => s.capital },
  { k:'runFor',   label:'运行天数', get:s => parseFloat(s.runFor) || 0 },
];
...
function ScreenLiveStrats() {
  ...
  const [sortK, setSortK] = React.useState(null);
  ...
  const [needPause, setNeedPause]       = React.useState(null);  // strat user tried to delete while running

  // 'all' shows everything EXCEPT stopped (stopped lives in its own bucket)
  const baseList = strats.filter(s => {
    if (filter === 'all')     return s.status !== 'stopped';
    return s.status === filter;
  });
  const sortDef = LS_SORTS.find(s => s.k === sortK);
  const list = (!sortDef || !sortDir) ? baseList : baseList.slice().sort((a,b) => {
    const d = sortDef.get(a) - sortDef.get(b);
  ...
  const onToggle = (s) => {
    ...
    if (s.status === 'stopped') {
      ...
      // running → paused: if there's an open position, ask the user how to handle it
  ...
  const onDelete = (s, opts = {}) => {
    const i = window.__qfLiveStrats.findIndex(x => x.id === s.id);
    if (i < 0) { setConfirmDel(null); setMenuFor(null); return; }
    if (s.status === 'stopped' || opts.permanent) {
      // permanent delete — drop the entry entirely
      window.__qfLiveStrats.splice(i, 1);
    } else {
      // soft delete — move to stopped, keep history for 30 days
  ...
  return (
    <div style={{height:'100%', display:'flex', flexDirection:'column', background:M.bg, position:'relative'}}>
      ...
      <MTopBar
        ...
        right={
          <button onClick={() => setSortOpen(true)} style={{
            ...
            background: sortDir ? M.violetSoft : 'transparent',
            border:0,
            color: sortDir ? M.violet : M.mid, cursor:'pointer',
            display:'flex', alignItems:'center', justifyContent:'center',
          }}>
            <Ico d={ICONS.filter} w={20} sw={1.8}/>
            {sortDir && (
              <span style={{
                position:'absolute', top:6, right:6, width:6, height:6,
                borderRadius:'50%', background:M.violet,
              }}/>
            )}
          </button>
        }
        ...
        {list.length === 0 ? (
          ...
          list.map(s => (
            <LsStratCard
              ...
              onAskDelete={() => {
                setMenuFor(null);
                // running strats must be paused first — show the hint sheet
                if (s.status === 'running') { setNeedPause(s); return; }
                setConfirmDel(s);
              }}
            />
          ))
      ...
      {/* pause-with-position sheet — running strats that have a live position */}
      {closeWithPos && (
        <LsCloseWithPositionSheet
          ...
          onConfirm={(mode) => {
            ...
            const pos = getPosition(s);
            if (mode === 'natural') {
              // keep running until SL/TP triggers, then auto-pause
              window.__qfLiveStrats[i] = {
                ...s, status:'running',
                statusNote: '暂停待生效 · 等待止损/止盈触发',
              };
            } else if (mode === 'keep') {
              // pause the bot, position kept on exchange — user handles manually
              window.__qfLiveStrats[i] = {
                ...s, status:'paused',
                statusNote: '已暂停 · 持仓已转手动管理',
              };
            } else {
              // market — realize current PnL into total, then pause
              const realized = pos ? pos.pnl : 0;
              window.__qfLiveStrats[i] = {
                ...s, status:'paused',
                statusNote: `已暂停 · 已平仓 ${realized >= 0 ? '+' : '-'}$${Math.abs(realized).toFixed(2)}`,
      ...
      <LsDeleteSheet
        ...
        onConfirm={(opts) => onDelete(confirmDel, opts)}
      />

      {/* "需要先暂停" hint — blocks delete on a still-running strategy */}
      <LsNeedPauseSheet
        open={!!needPause}
        name={needPause?.name}
        onCancel={() => setNeedPause(null)}
        onPause={() => {
          const s = needPause;
          setNeedPause(null);
          if (s) onToggle(s);   // triggers pause (or pause-with-position sheet)
        }}
      />

      {/* ─── 筛选 & 排序 — bottom sheet (funnel button) ─── */}
      {sortOpen && (
        <div
          ...
          <div
            ...
            {/* 状态 — pills (synced with inline filter) */}
            <div style={{fontSize:11, color:M.mid, marginBottom:6}}>状态</div>
            <div style={{display:'flex', flexWrap:'wrap', gap:6, marginBottom:16}}>
              {LS_FILTERS.map(f => {
                const on = filter === f.k;
                const count = f.k === 'all'
                  ? strats.filter(s => s.status !== 'stopped').length
                  : strats.filter(s => s.status === f.k).length;
                return (
                  <button key={f.k} onClick={() => setFilter(f.k)} style={{
                    height:30, padding:'0 13px', borderRadius:999, fontSize:12,
                    ...
                    {f.label}
                    <span style={{
                      fontFamily:M.mono, fontSize:10.5, fontWeight:600,
                      color: on ? 'rgba(255,255,255,0.75)' : M.dim,
                    }}>{count}</span>
                  </button>
                );
              })}
            ...
            {/* 排序指标 — pills */}
            <div style={{fontSize:11, color:M.mid, marginBottom:6}}>排序指标</div>
            <div style={{display:'flex', flexWrap:'wrap', gap:6, marginBottom:16}}>
              {LS_SORTS.map(s => {
                const on = sortK === s.k;
                return (
            ...
            <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:6}}>
              {[
                ...
                const on = (d.k == null) ? !sortDir : sortDir === d.k;
                return (
                  <button key={String(d.k)} onClick={() => {
                    if (d.k == null) { setSortDir(null); return; }
                    setSortDir(d.k);
                    setSortK(k => k || LS_SORTS[0].k);
                  }} style={{
            ...
            <button onClick={() => setSortOpen(false)} style={{
            ...
            }}>查看 {list.length} 个策略</button>
          </div>
        </div>
      )}

      {/* strategy action drawer (查看详情 / 暂停 / 删除) */}
      {(() => {
        const ms = strats.find(x => x.id === menuFor);
        if (!ms) return null;
        const canStart  = ms.status === 'paused' || ms.status === 'warning';
        const isStopped = ms.status === 'stopped';
        ...
        const items = [
          { icon:'M4 7h12M4 12h16M4 17h10', label:'查看详情',
            onClick: () => onView(ms) },
          { icon: isStopped ? RESTORE_D : (canStart ? ICONS.play : PAUSE_D),
            ...
            onClick: () => { setMenuFor(null); onToggle(ms); } },
          { icon: TRASH_D, label: isStopped ? '永久删除' : '删除策略', tone:'danger',
            onClick: () => {
              setMenuFor(null);
              if (ms.status === 'running') { setNeedPause(ms); return; }
              setConfirmDel(ms);
            } },
        ];
        return (
          <div onClick={() => setMenuFor(null)} style={{
            ...
            <div onClick={(e)=>e.stopPropagation()} style={{
              width:'100%', background:M.elev,
              borderRadius:'20px 20px 0 0',
              display:'flex', flexDirection:'column',
              boxShadow:'0 -16px 48px rgba(15,11,34,0.32)',
              animation:'ls-slide-up .22s ease-out',
            }}>
              <div style={{width:42, height:4, borderRadius:2, background:M.border, margin:'10px auto 6px'}}/>
              <div style={{
                padding:'4px 16px 8px', fontSize:13, fontWeight:600, color:M.dim,
                whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis',
              }}>{ms.name}</div>
              <div style={{padding:'2px 8px 6px'}}>
                {items.map((it, i) => {
                  const col = it.tone === 'danger' ? M.danger : it.tone === 'ok' ? M.ok : M.text;
                  return (
                    <button key={i} onClick={it.onClick} style={{
                      ...
                      <Ico d={it.icon} w={18} sw={1.9}/>
                      <span style={{fontSize:15, fontWeight:600, color:col}}>{it.label}</span>
                    </button>
                  );
                })}
              </div>
              <div style={{
                ...
                <button onClick={() => setMenuFor(null)} style={{
...
/* "needs-pause" prompt — bottom drawer shown when user tries to delete a still-running strat */
function LsNeedPauseSheet({ open, name, onCancel, onPause }) {
  if (!open) return null;
  return (
      ...
      <div onClick={(e)=>e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <span style={{
            ...
            <Ico d="M12 9v4M12 17h.01M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"
              w={15} sw={1.8}/>
          </span>
        ...
        <div style={{padding:'14px 16px 12px'}}>
          <div style={{fontSize:13, color:M.mid, lineHeight:1.65}}>
            「<strong style={{color:M.text, fontWeight:600}}>{name}</strong>」当前仍在运行,
            为避免误删持仓和正在进行的交易,请<strong style={{color:M.text, fontWeight:600}}>先暂停</strong>策略后再删除。
          </div>
        </div>
        ...
        <div style={{
          ...
          <button onClick={onPause} style={{
            flex:1.4, height:44, borderRadius:10, border:0,
            background:M.violetGrad, color:'#fff',
            fontSize:13, fontWeight:700, cursor:'pointer',
            fontFamily:'inherit',
            boxShadow:'0 6px 14px -8px rgba(124,92,255,0.55)',
            display:'flex', alignItems:'center', justifyContent:'center', gap:6,
          }}>
            <Ico d="M6 4h4v16H6zM14 4h4v16h-4z" w={13} fill="#fff" sw={0}/>
            去暂停策略
          </button>
        </div>
      </div>
...
function LsDeleteSheet({ open, name, stopped, onCancel, onConfirm }) {
  ...
  React.useEffect(() => {
    if (open) { setShowDanger(false); setPermanent(false); }
  }, [open]);
  if (!open) return null;
  // stopped strats are already a permanent delete; never show the opt-in
  const hardDelete = stopped || permanent;
  return (
    <div onClick={onCancel} style={{
      ...
      <div onClick={(e)=>e.stopPropagation()} style={{
        ...
        <div style={{padding:'14px 16px 12px'}}>
          <div style={{fontSize:13, color:M.mid, lineHeight:1.65}}>
            {stopped ? (
              ...
              <>「<strong style={{color:M.text, fontWeight:600}}>{name}</strong>」会立即停止运行,
                历史记录将<strong style={{color:M.danger, fontWeight:600}}>不予保留</strong>,此操作不可撤销。</>
            ) : (
              <>「<strong style={{color:M.text, fontWeight:600}}>{name}</strong>」会立即停止运行,
          ...
          {/* opt-in permanent delete — only for soft delete (running strats) */}
          {!stopped && (
            <div style={{marginTop:10}}>
              <button onClick={() => setShowDanger(v => !v)} style={{
                background:'transparent', border:0, padding:0, cursor:'pointer',
                color:M.violet, fontSize:12, fontWeight:500, fontFamily:'inherit',
              ...
              {showDanger && (
                <div style={{
                  ...
                  <div>
                    <strong>立即永久删除</strong> · 删除后将<strong>不</strong>保留 30 天历史。
                    通常仅在合规或隐私要求时选择。
                  </div>
                  <label style={{
                    ...
                    <input
                      ...
                      style={{accentColor:M.danger, width:14, height:14, margin:0, cursor:'pointer'}}/>
        ...
        {/* footer — extra bottom inset clears the iOS home indicator (34px) */}
        <div style={{
          padding:'14px 16px 40px',
          borderTop:`1px solid ${M.borderSoft}`,
          display:'flex', alignItems:'center', gap:10, background:M.elev,
          ...
          <button onClick={() => onConfirm({ permanent: hardDelete })} style={{
            flex:1.4, height:44, borderRadius:10, border:0,
            background:M.danger, color:'#fff',
            fontSize:13, fontWeight:700, cursor:'pointer',
            fontFamily:'inherit',
            boxShadow:`0 6px 14px -8px ${M.danger}`,
          }}>{hardDelete ? '永久删除' : '删除策略'}</button>
        </div>
      </div>
      <style>{`
...
function LsCloseWithPositionSheet({ s, position, onCancel, onConfirm }) {
  ...
  const modes = [
    ...
    },
    {
      ...
      label: '保留持仓,仅暂停策略',
      warn: true,
      desc: '策略不再监控,持仓需要你在交易所手动管理,恢复后可继续接管。',
      effect: (
      ...
      <div className="m-sheet" onClick={(e)=>e.stopPropagation()} style={{
        ...
        <div style={{padding:'12px 20px 10px'}}>
          <div style={{
            ...
            <div style={{fontSize:16, fontWeight:700, color:M.text}}>暂停策略</div>
          </div>
          <div style={{fontSize:12, color:M.mid, lineHeight:1.6}}>
        ...
        <div style={{
          ...
          <Ico d="M12 8v4m0 4h.01M12 22a10 10 0 1 1 0-20 10 10 0 0 1 0 20z"
            w={13} sw={1.7}/>
          <span>
            暂停后策略保留全部参数和历史,随时可在「已暂停」中<strong style={{color:M.text}}>一键恢复</strong>。
          </span>
        </div>
...
function ScreenLiveStratDetail() {
  ...
  const [needPause, setNeedPause]       = React.useState(false);
  ...
  return (
    <div style={{height:'100%', display:'flex', flexDirection:'column', background:M.bg, position:'relative'}}>
      ...
      <MTopBar
        ...
        right={
          <button onClick={() => setMenuOpen(o => !o)} style={{
            width:36, height:36, border:0, background:'transparent', padding:0,
      ...
      {/* top-right action menu — bottom drawer */}
      {menuOpen && (
            ...
            <div style={{padding:'2px 8px 6px'}}>
              {[
                ...
                { icon:'M3 6h18M8 6V4a2 2 0 012-2h4a2 2 0 012 2v2M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6',
                  ...
                  onClick: () => {
                    setMenuOpen(false);
                    if (s.status === 'running') { setNeedPause(true); return; }
                    setConfirmDel(true);
                  } },
              ].map((it, i) => {
                const col = it.tone === 'danger' ? M.danger : it.tone === 'ok' ? M.ok : M.text;
      ...
      <LsNeedPauseSheet
        open={!!needPause}
        name={s.name}
        onCancel={() => setNeedPause(false)}
        onPause={() => {
          setNeedPause(false);
          onToggle();   // running → paused (may open pause-with-position sheet)
        }}
      />
    </div>
  );
    ...
    <Card p="16px">
      ...
      <div style={{
        ...
        {[
          ...
          <div key={k} style={{textAlign: i===0 ? 'left' : i===1 ? 'center' : 'right'}}>
            <div style={{fontSize:10, color:M.dim, marginBottom:3}}>{k}</div>
            <div style={{
