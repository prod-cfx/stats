...
- 将配套的 AI / LLM 提示词一并收敛到新决策模型，避免旧 prompt 继续输出模板化误追问
...
- LLM 提示词只负责根据当前决策结果生成自然语言交互
- 禁止提示词绕过决策层，自行补入出场、止盈止损、方向、周期等核心语义
...
### 9.6 LLM Prompt / Planner Prompt 配套调整

本次实现需要同步优化与澄清、确认、生成相关的提示词，但优化目标是“服从新决策层”，不是“靠提示词兜底业务规则”。

具体要求：

- `ASK_CLARIFY` 场景
  - 只允许围绕 `blockingReasons[0]` 对应问题发问
  - 禁止沿用旧模板习惯性追加“请补充出场规则 / 止盈 / 止损 / 主周期”等无关追问
  - 必须先回显当前已理解的策略，再追问本轮唯一问题

- `CONFIRM_INFERRED` 场景
  - 必须明确区分“你明确给出的内容”和“系统推断内容”
  - 禁止在未确认前把推断结果当成既定事实继续下游生成

- `DIRECT_COMPILE` 场景
  - 提示词不得因为旧 checklist 缺项而反向触发追问
  - 在程序已判定可直接生成时，prompt 只负责进入生成链路

- Planner / conversation prompt
  - 改成消费 `StrategyDecision` 与结构化证据，而不是继续面向零散 `missing_*` 规则写死行为
  - 明确禁止用“大概率”“通常”“默认就按”这类模糊语言偷偷补关键交易语义
...
- assistant prompt
- 关键 blocking reason
- 提示词输出是否严格服从 `StrategyDecision`，不额外追加模板化误追问
...
### 15.4 风险：程序已收口，但旧提示词仍然输出错误追问

缓解：

- 将澄清 / 确认 / 生成相关提示词纳入本次同一实现范围
- 为 prompt 输出增加集成测试断言，确保其服从 `StrategyDecision`
- 禁止在 prompt 中保留旧的“缺少出场规则 / 缺少止盈止损 / 缺少主周期”硬模板
